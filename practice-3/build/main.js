...
const fetch_data_js_1 = require("./fetch-data.js");
const display_weather_js_1 = require("./display-weather.js");
    ...
    (async () => {
        try {
            if (navigator.geolocation) {
                ...
                city = await (0, fetch_data_js_1.reverseGeocode)(latitude, longitude);
                sessionStorage.setItem("currentCity", city);
            }
            else
                console.log("Geolocation is not supported by this browser.");
            // Now that we have the actual current location, fetch the weather
            const weatherData = await (0, fetch_data_js_1.getWeather)(city);
            sessionStorage.setItem("weatherData", JSON.stringify(weatherData));
            (0, display_weather_js_1.displayWeather)(weatherData);
        }
        catch (error) {
